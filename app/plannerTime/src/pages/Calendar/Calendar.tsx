...
function CalendarPage() {
    ...
    const filteredEvents = selectedDay
      ? events.filter((event) => {
          ...
          return Number.parseInt(day) === selectedDay
        })
      : events
        ...
        <main className="flex-1 py-8">
          <div className="container mx-auto px-4">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <Card>
                <h2 className="text-xl font-semibold mb-4">Visualização Mensal</h2>
              ...
              <Card>
                <div className="flex justify-between items-center mb-4">
                  ...
                  {selectedDay && (
                    <button onClick={() => setSelectedDay(null)} className="text-sm text-blue-600 hover:underline">
                      Ver todos
                    </button>
                  )}
                </div>
                ...
                {filteredEvents.length > 0 ? (
                  <div className="space-y-4">
                    {filteredEvents.map((event) => (
                      <div key={event.id} className={`p-3 border rounded-lg ${event.color}`}>
                  ...
                  <div className="p-8 text-center text-gray-500">
                    {selectedDay ? `Não há eventos para o dia ${selectedDay}/04` : "Não há eventos próximos"}
                  </div>
                )}
